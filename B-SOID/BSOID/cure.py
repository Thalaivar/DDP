...
class CURE:
    def __init__(self, k, n_parts, k_per_part, **cluster_args):
    ...
    def process(self, data):
        ...
        while len(self.heap_) > self.desired_clusters:
            ...
            w.closest = self.heap_[0]
            w.distance = self._distance_between_clusters(w, w.closest)
            for c in self.heap_:
    ...
    def _create_heap(self, data):
        ...
        for i in range(len(self.heap_)):
            ...
            # assign representative point and idx to each cluster
            self.heap_[i].rep_idx = [i]
            self.heap_[i]._exemplars_from_data()
    ...
    def _distance_between_clusters(self, u: cure_cluster, v:cure_cluster):
        ...
        pairwise_rep = []
        permut = itertools.permutations(larger_cluster.rep, len(smaller_cluster.rep))
        for comb in permut:
            pairs = zip(comb, smaller_cluster.rep)
            pairwise_rep.append(list(pairs))
        
        min_dist = np.inf
        for pair in pairwise_rep:
            dist = np.linalg.norm(pair[0][0] - pair[0][1])
    ...
    def _closest_cluster(self, cluster: cure_cluster, distance: float):
        ...
        for p in cluster.rep:
            ...
            same_point_idx = np.where(dist == 0.0)[0]
            idx = np.delete(idx, same_point_idx)
            dist = np.delete(dist, same_point_idx)
            if idx.size == 0:
                # no points within distance
                return (None, None)
            elif not np.any(np.array(self.active_points)[idx] >= 0):
                # no active points within distance
                return (None, None)
            else:
                # consider only active points
                active_idx = [i for i in range(idx.size) if self.active_points[idx[i]] >= 0]
                idx, dist = idx[active_idx], dist[active_idx]
    ...
    def _merge_clusters(self, u: cure_cluster, v: cure_cluster):
        ...
        scattered_points, retained_idx = well_scattered_points(w.n_rep, w.mean, np.array(scattered_points))
        ...
        for idx in retained_idx:
            if idx < len(u.rep):
                w.rep_idx.append(u.rep_idx[idx])
            else:
                w.rep_idx.append(v.rep_idx[idx - len(u.rep)])
